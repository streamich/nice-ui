...
(function() {
  ...
  ABCJS.startAnimation = function(h, j, q) {
    if (h.getElementsByClassName === undefined) {
      console.error(
        'ABCJS.startAnimation: The first parameter must be a regular DOM element. (Did you pass a jQuery object or an ID?)',
      );
      return;
    }
    if (j.getBeatLength === undefined) {
      console.error(
        'ABCJS.startAnimation: The second parameter must be a single tune. (Did you pass the entire array of tunes?)',
      );
      return;
    }
    if (q.showCursor) {
...
(function() {
  ...
  ABCJS.renderMidi = function(c, d, b, f, e) {
    function g(k, h) {
      ...
      var i = window.ABCJS.midi.create(h, f);
      var j = h.metaText.title;
      if (j) {
        j = j.toLowerCase().replace(/\W/g, '_');
      }
      k.innerHTML = '<a download="' + j + '.midi" href="' + i + '">download midi</a>';
    }
    return a(g, c, d, b, e);
  };
})();
...
window.ABCJS.data.Tune = function() {
    ...
    for (this.lineNum = 0; this.lineNum < this.lines.length; this.lineNum++) {
      if (this.lines[this.lineNum].staff) {
        for (this.staffNum = 0; this.staffNum < this.lines[this.lineNum].staff.length; this.staffNum++) {
          ...
          for (
            this.voiceNum = 0;
            this.voiceNum < this.lines[this.lineNum].staff[this.staffNum].voices.length;
            this.voiceNum++
          ) {
  ...
  this.appendStartingElement = function(f, a, h, c) {
    ...
    if (this.lines[this.lineNum].staff.length <= this.staffNum) {
      this.lines[this.lineNum].staff[this.staffNum] = {};
      this.lines[this.lineNum].staff[this.staffNum].clef = window.ABCJS.parse.clone(
        this.lines[this.lineNum].staff[0].clef,
      );
      this.lines[this.lineNum].staff[this.staffNum].key = window.ABCJS.parse.clone(
        this.lines[this.lineNum].staff[0].key,
      );
      this.lines[this.lineNum].staff[this.staffNum].meter = window.ABCJS.parse.clone(
        this.lines[this.lineNum].staff[0].meter,
      );
      this.lines[this.lineNum].staff[this.staffNum].workingClef = window.ABCJS.parse.clone(
  ...
  this.addSeparator = function(c, b, a) {
    this.pushLine({
      separator: {spaceAbove: c, spaceBelow: b, lineLength: a},
    });
  };
  ...
  this.addCentered = function(a) {
    this.pushLine({text: [{text: a, center: true}]});
  };
  ...
  this.startNewLine = function(e) {
    ...
    var c = function(j) {
      ...
      if (j.style) {
        d.appendElement('style', null, null, {head: j.style});
      }
      if (j.stem) {
        d.appendElement('stem', null, null, {direction: j.stem});
      } else {
        if (d.voiceNum > 0) {
          ...
          d.appendElement('stem', null, null, {direction: 'down'});
        }
      }
      if (j.scale) {
        d.appendElement('scale', null, null, {size: j.scale});
      }
    };
    var a = function(f) {
      ...
      c(f);
      if (f.part) {
        d.appendElement('part', f.startChar, f.endChar, {title: f.part});
      }
  ...
  this.getCurrentVoice = function() {
    if (
      this.lines[this.lineNum] !== undefined &&
  ...
  this.setCurrentVoice = function(c, b) {
    ...
    for (var a = 0; a < this.lines.length; a++) {
      if (this.lines[a].staff) {
        if (
          this.lines[a].staff[c] === undefined ||
          this.lines[a].staff[c].voices[b] === undefined ||
          !this.containsNotes(this.lines[a].staff[c].voices[b])
        ) {
          this.lineNum = a;
          return;
        }
...
(function() {
  var a = 480 * 4;
  window.ABCJS.midi.create = function(l, m) {
    var f = window.ABCJS.midi.sequence(l, m);
    var e = window.ABCJS.midi.flatten(f);
    var c = window.ABCJS.midi.rendererFactory(false);
    if (e.instrument !== undefined) {
      c.setInstrument(e.instrument);
    }
    if (e.channel !== undefined) {
      c.setChannel(e.channel);
    }
    var k = l.metaText ? l.metaText.title : undefined;
    if (k && k.length > 128) {
      k = k.substring(0, 124) + '...';
    }
    c.setGlobalInfo(e.tempo, k);
    for (var h = 0; h < e.tracks.length; h++) {
      c.startTrack();
      for (var g = 0; g < e.tracks[h].length; g++) {
        var d = e.tracks[h][g];
        switch (d.cmd) {
          case 'instrument':
            c.setInstrument(d.instrument);
            break;
          case 'start':
            c.startNote(b(d.pitch), d.volume);
            break;
          case 'stop':
            c.endNote(b(d.pitch), 0);
            break;
          case 'move':
            c.addRest(d.duration * a);
            break;
          default:
            console.log('MIDI create Unknown: ' + d.cmd);
        }
      }
      c.endTrack();
    }
...
(function() {
  ...
  var s;
  var c = 1 / 128;
  window.ABCJS.midi.flatten = function(N) {
    m = [];
    q = [0, 0, 0, 0, 0, 0, 0];
    l = 0;
    j = false;
    ...
    return {tempo: h, instrument: x, channel: w, tracks: H};
  };
  var p = ['break', '(break)', 'no chord', 'n.c.', 'tacet'];
  function D(P) {
    if (k || !P.chord || P.chord.length === 0) {
      return null;
    }
    for (var N = 0; N < P.chord.length; N++) {
      var O = P.chord[N];
      if (O.position === 'default') {
        return O.name;
      }
      if (p.indexOf(O.name.toLowerCase()) >= 0) {
        return 'break';
      }
    }
    return null;
  ...
  function i(P) {
    ...
    if (R) {
      ...
      if (X) {
        if (r.length === 0) {
          r.push({cmd: 'instrument', instrument: 2});
          var O = 0;
          ...
          if (O > 0) {
            r.push({cmd: 'move', duration: O});
          }
        }
        M = X;
        n.push({chord: M, beat: s});
      }
    }
    if (P.startTriplet) {
      if (P.startTriplet === 2) {
        ...
        B = (P.startTriplet - 1) / P.startTriplet;
      }
    }
    var Q = P.duration * B;
    s += Q;
    ...
    if (P.pitches) {
      ...
      var T = [];
      for (var S = 0; S < P.pitches.length; S++) {
        var W = P.pitches[S];
        var N = K(W);
        T.push({pitch: N, startTie: W.startTie});
        if (!e['' + N]) {
          G.push({cmd: 'start', pitch: N, volume: 64});
        }
        if (W.startTie) {
          e['' + N] = true;
        } else {
      ...
      G.push({cmd: 'move', duration: Q - c});
      o = G.length - 1;
      for (var aa = 0; aa < T.length; aa++) {
        if (!e['' + T[aa].pitch]) {
          G.push({cmd: 'stop', pitch: T[aa].pitch});
        }
      }
      G.push({cmd: 'move', duration: c});
    } else {
      if (P.rest) {
        G.push({cmd: 'move', duration: Q});
      }
    }
    if (P.endTriplet) {
      B = 1;
  ...
  function E(Q) {
    ...
    for (var N = 0; N < Q.accidentals.length; N++) {
      var R = Q.accidentals[N];
      var S = R.acc === 'sharp' ? 1 : R.acc === 'natural' ? 0 : -1;
      var P = R.note.toLowerCase();
      var O = g(P.charCodeAt(0) - 'c'.charCodeAt(0));
      T[O] += S;
    }
    return T;
  }
  ...
  function C(O, R) {
    ...
    for (P = 0; P < O.length; P++) {
      Q = O[P];
      N.push({pitch: Q.pitch, duration: (Q.duration / I) * T});
    }
    return N;
  }
  function b(O, R, S, N) {
    for (var Q = 0; Q < O.length; Q++) {
      var P = K(O[Q]);
      if (P !== N) {
        G.push({cmd: 'start', pitch: P, volume: 64});
      }
      G.push({cmd: 'move', duration: O[Q].duration});
      if (P !== N) {
        G.push({cmd: 'stop', pitch: P});
      }
      if (!R) {
        G[o].duration -= O[Q].duration;
      }
      S -= O[Q].duration;
    }
    return S;
  ...
  var A = {A: -27, B: -25, C: -24, D: -22, E: -20, F: -19, G: -17};
  function J(P) {
    if (P.length === 0) {
      return undefined;
    }
  ...
  function L(N, O) {
    if (N !== undefined) {
      r.push({cmd: 'start', pitch: N, volume: 64});
    }
    r.push({cmd: 'move', duration: O / 2});
    if (N !== undefined) {
      r.push({cmd: 'stop', pitch: N});
    }
    r.push({cmd: 'move', duration: O / 2});
  }
  function v(N, O) {
    for (var P = 0; P < N.length; P++) {
      r.push({cmd: 'start', pitch: N[P], volume: 48});
    }
    r.push({cmd: 'move', duration: O / 2});
    for (P = 0; P < N.length; P++) {
      r.push({cmd: 'stop', pitch: N[P]});
    }
    r.push({cmd: 'move', duration: O / 2});
  }
  var y = {
  ...
  function z() {
    ...
    var W = 1 / X;
    var V = y[T + '/' + X];
    var O = parseInt(T, 10) / parseInt(X, 10);
    var P = Math.abs(O - s);
    if (!V || P > 0.0078125) {
      V = [];
      var aa = s / W;
    ...
    if (n.length === 0) {
      n.push({beat: 0, chord: M});
    }
    if (n[0].beat !== 0 && M) {
      n.unshift({beat: 0, chord: M});
    }
    if (n.length === 1) {
...
(function() {
  ...
  g.prototype.setTempo = function(i) {
    this.javamidi.setTempo(i);
    this.qtmidi.setTempo(i);
  };
  ...
  g.prototype.setInstrument = function(i) {
    this.javamidi.setInstrument(i);
    this.qtmidi.setInstrument(i);
  };
  g.prototype.startNote = function(k, i, j) {
    this.javamidi.startNote(k, i, j);
    this.qtmidi.startNote(k, i, j);
  };
  g.prototype.endNote = function(j, i) {
    this.javamidi.endNote(j, i);
    this.qtmidi.endNote(j, i);
  };
  g.prototype.addRest = function(i) {
    this.javamidi.addRest(i);
    this.qtmidi.addRest(i);
  };
  g.prototype.embed = function(i) {
    this.javamidi.embed(i);
    this.qtmidi.embed(i, true);
  };
  function b(i) {
    this.playlist = [];
    this.trackcount = 0;
  ...
  b.prototype.setTempo = function(i) {
    this.tempo = i;
  };
  ...
  b.prototype.setInstrument = function(i) {
    this.instrument = i;
    this.midiapi.setInstrument(i);
  };
  b.prototype.setChannel = function(i) {
    this.channel = i;
    this.midiapi.setChannel(i);
  };
  ...
  b.prototype.startNote = function(l, j, k) {
    ...
    this.playlist.splice(this.playlistpos, 0, {
      time: this.timecount,
      funct: function() {
        i.midiapi.playNote(l);
        i.midiwriter.notifySelect(k);
      },
    });
  };
  b.prototype.endNote = function(k, j) {
    this.timecount += j;
    this.updatePos();
    var i = this;
    this.playlist.splice(this.playlistpos, 0, {
      time: this.timecount,
      funct: function() {
        i.midiapi.stopNote(k);
      },
    });
  };
  b.prototype.addRest = function(i) {
    this.silencelength += i;
  };
  b.prototype.embed = function(j) {
  ...
  d.prototype.setTempo = function(i) {
    if (this.trackcount === 0) {
      this.startTrack();
      this.track += '%00%FF%51%03' + c(Math.round(60000000 / i), 6);
      this.endTrack();
    }
  };
  d.prototype.setGlobalInfo = function(k, j) {
    if (this.trackcount === 0) {
      this.startTrack();
      this.track += '%00%FF%51%03' + c(Math.round(60000000 / k), 6);
      if (j) {
        this.track += '%00%FF%03' + c(j.length, 2);
        for (var l = 0; l < j.length; l++) {
          this.track += c(j.charCodeAt(l), 2);
        }
      }
      this.endTrack();
    }
  ...
  d.prototype.endTrack = function() {
    var i = c(this.track.length / 3 + 4, 8);
    this.track = 'MTrk' + i + this.track + '%00%FF%2F%00';
    this.trackstrings += this.track;
  };
  d.prototype.setInstrument = function(i) {
    if (this.track) {
      this.track = '%00%C0' + c(i, 2) + this.track;
    } else {
      this.track = '%00%C0' + c(i, 2);
    }
    this.instrument = i;
  };
  d.prototype.setChannel = function(i) {
    this.channel = i - 1;
    this.noteOnAndChannel = '%9' + this.channel.toString(16);
  };
  d.prototype.startNote = function(j, i) {
    ...
    this.track += '%' + j.toString(16) + c(i, 2);
  };
  d.prototype.endNote = function(j, i) {
    this.track += e(this.silencelength + i);
    this.silencelength = 0;
    this.track += '%' + j.toString(16) + '%00';
  };
  d.prototype.addRest = function(i) {
    this.silencelength += i;
  };
  d.prototype.getData = function() {
    return 'data:audio/midi,MThd%00%00%00%06%00%01' + c(this.trackcount, 4) + '%01%e0' + this.trackstrings;
  };
  d.prototype.embed = function(j, i) {
    var l = this.getData();
    var k = a(document.createElement('a'), {href: l});
    k.innerHTML = 'download midi';
    j.insertBefore(k, j.firstChild);
    if (i) {
      return;
    }
    var m = a(document.createElement('embed'), {
      src: l,
  ...
  function e(p) {
    ...
    var o = l.toString(16).length;
    o += o % 2;
    return c(l, o);
  }
  window.ABCJS.midi.rendererFactory = function(i) {
    return i ? new g(new b(this), new d()) : new d();
  };
  window.ABCJS.midi.initializeJava = function() {
    h = document.MIDIPlugin;
    setTimeout(function() {
      try {
        h.openPlugin();
      } catch (j) {
        var i = document.createElement('a');
        i.href = 'http://java.sun.com/products/java-media/sound/soundbanks.html';
        i.target = '_blank';
        i.appendChild(document.createTextNode('Download Soundbank'));
...
(function() {
  window.ABCJS.midi.sequence = function(c, d) {
    d = d || {};
    var f = d.qpm || 180;
    var l = d.program || 2;
    var t = d.transpose || 0;
    var I = d.channel || 0;
    var D = c.formatting.bagpipes;
    if (D) {
      l = 71;
    }
    if (c.formatting.midi) {
      if (c.formatting.midi.transpose) {
        t = c.formatting.midi.transpose;
      }
      if (c.formatting.midi.program && c.formatting.midi.program.program) {
        l = c.formatting.midi.program.program;
      }
      if (c.formatting.midi.channel) {
        I = c.formatting.midi.channel;
      }
    }
    if (c.metaText.tempo) {
      f = b(c.metaText.tempo);
    }
    var E = [];
    if (D) {
      E.push({el_type: 'bagpipes'});
    }
    E.push({el_type: 'instrument', program: l});
    if (I) {
      E.push({el_type: 'channel', channel: I});
    }
    if (t) {
      E.push({el_type: 'transpose', transpose: t});
    }
    E.push({el_type: 'tempo', qpm: f});
    var g = [];
    var G = [];
    var u = [];
    for (var A = 0; A < c.lines.length; A++) {
      ...
      if (p.staff) {
        ...
        for (var z = 0; z < h.length; z++) {
          var n = h[z];
          for (var y = 0; y < n.voices.length; y++) {
            var H = n.voices[y];
            if (!g[o]) {
              g[o] = [].concat(E);
            }
            if (n.key) {
              if (n.key.root === 'HP') {
                g[o].push({
                  el_type: 'key',
                  accidentals: [{acc: 'natural', note: 'g'}, {acc: 'sharp', note: 'f'}, {acc: 'sharp', note: 'c'}],
                });
              } else {
                g[o].push({el_type: 'key', accidentals: n.key.accidentals});
              }
            }
            if (n.meter) {
              g[o].push(a(n.meter));
            }
            if (n.clef && n.clef.transpose) {
              n.clef.el_type = 'clef';
              g[o].push({el_type: 'transpose', transpose: n.clef.transpose});
            }
            var x = 0;
              ...
              switch (C.el_type) {
                ...
                case 'key':
                  if (C.root === 'HP') {
                    ...
                    g[o].push({el_type: 'key', accidentals: C.accidentals});
                  }
                  break;
                case 'meter':
                  g[o].push(a(C));
                ...
                case 'clef':
                  if (C.transpose) {
                    g[o].push({el_type: 'transpose', transpose: C.transpose});
                  }
                  break;
                case 'tempo':
                  f = b(C);
                  g[o].push({el_type: 'tempo', qpm: f});
                  break;
                case 'bar':
                  if (x > 0) {
                    g[o].push({el_type: 'bar'});
                  }
                  x = 0;
                  var m = C.type === 'bar_right_repeat' || C.type === 'bar_dbl_repeat';
                  var q = C.startEnding ? true : false;
                  var F =
                    C.type === 'bar_left_repeat' ||
                    C.type === 'bar_dbl_repeat' ||
                    C.type === 'bar_thick_thin' ||
                    C.type === 'bar_thin_thick' ||
                    C.type === 'bar_thin_thin' ||
                    C.type === 'bar_right_repeat';
                  if (m) {
                    var w = G[o];
                    if (!w) {
                      w = 0;
                ...
                case 'scale':
                  break;
                default:
                  console.log('MIDI: element type ' + C.el_type + ' not handled.');
              }
            }
            o++;
          }
  ...
  function a(c) {
    switch (c.type) {
      ...
      case 'specified':
        return {el_type: 'meter', num: c.value[0].num, den: c.value[0].den};
      default:
...
window.ABCJS.parse.Parse = function() {
  ...
  var i = [['<(', 'crescendo('], ['<)', 'crescendo)'], ['>(', 'diminuendo('], ['>)', 'diminuendo)']];
  var k = function(G, I) {
    ...
    if (J !== undefined) {
      ...
      if (
        window.ABCJS.parse.detect(t, function(K) {
          return J === K;
        })
      ) {
        return [1, J];
        ...
        if (
          window.ABCJS.parse.detect(E, function(K) {
            return J === K;
          })
        ) {
          ...
          if (
            window.ABCJS.parse.detect(b, function(K) {
            ...
            if (
              !window.ABCJS.parse.detect(C.ignoredDecorations, function(K) {
                return J === K;
              })
            ) {
    ...
    switch (G.charAt(I)) {
      ...
      case '+':
        ...
        if (
          window.ABCJS.parse.detect(v, function(K) {
            if (H[1] === K[0]) {
              H[1] = K[1];
              return true;
            } else {
        ...
        if (
          window.ABCJS.parse.detect(i, function(K) {
            if (H[1] === K[0]) {
              H[1] = K[1];
              return true;
            } else {
  ...
  var q = function(H, N) {
    ...
    var G = function(O) {
      ...
      if (P.length > 0) {
        ...
        M.push({syllable: r.translateString(P), divider: Q});
        J = false;
        return true;
      }
      return false;
      ...
      switch (N.charAt(I)) {
        ...
        case '-':
          if (!G(I) && M.length > 0) {
            ...
            M.push({skip: true, to: 'next'});
          }
          break;
        case '_':
          G(I);
          M.push({skip: true, to: 'slur'});
          break;
        case '*':
          G(I);
          M.push({skip: true, to: 'next'});
  ...
  var a = {x: 'invisible', y: 'spacer', z: 'rest', Z: 'multimeasure'};
  ...
  function z() {
    var H = {startChar: -1, endChar: -1};
    if (C.partForNextLine.length) {
      H.part = C.partForNextLine;
    }
    H.clef =
      C.currentVoice && C.staves[C.currentVoice.staffNum].clef !== undefined
        ? window.ABCJS.parse.clone(C.staves[C.currentVoice.staffNum].clef)
        : window.ABCJS.parse.clone(C.clef);
    ...
    C.partForNextLine = '';
    if (
      C.currentVoice === undefined ||
      (C.currentVoice.staffNum === C.staves.length - 1 &&
        C.staves[C.currentVoice.staffNum].numVoices - 1 === C.currentVoice.index)
    ) {
      if (C.barNumbers === 0) {
      ...
      if (H[0] > 0) {
        ...
        while (1) {
          ...
          if (ad[0] > 0) {
            ...
            if (L === false) {
              if (ad[2] === null && ad[3]) {
                I.chord.push({name: M, rel_position: ad[3]});
              } else {
                I.chord.push({name: M, position: ad[2]});
              }
            }
            Y += ad[0];
                ...
                if (P !== null) {
                  ...
                  if (Q.charAt(Y) === ' ') {
                    ...
                    if (Y < Q.length && Q.charAt(Y) === ']') {
                      ...
                      if (C.inTie) {
                        window.ABCJS.parse.each(I.pitches, function(ae) {
                          ae.endTie = true;
                        });
                        C.inTie = false;
                      }
...
(function() {
  ...
  var a = {gchordfont: true, measurefont: true, partsfont: true};
  var d = function(r) {
    switch (r) {
      case 'Arial-Italic':
        return {
    ...
    while (z.length) {
      ...
      switch (s) {
        case 'face':
          if (
            t ||
            (H !== 'utf' && F.type !== 'number' && H !== 'bold' && H !== 'italic' && H !== 'underline' && H !== 'box')
          ) {
  ...
  window.ABCJS.parse.parseDirective.parseFontChangeLine = function(t) {
    ...
    if (r.length > 1 && e.setfont) {
      ...
      for (var s = 1; s < r.length; s++) {
        if (r[s].charAt(0) === '0') {
          u.push({text: r[s].substring(1)});
        } else {
          if (r[s].charAt(0) === '1' && e.setfont[1]) {
            u.push({font: e.setfont[1], text: r[s].substring(1)});
          } else {
            if (r[s].charAt(0) === '2' && e.setfont[2]) {
              u.push({font: e.setfont[2], text: r[s].substring(1)});
            } else {
              if (r[s].charAt(0) === '3' && e.setfont[3]) {
                u.push({font: e.setfont[3], text: r[s].substring(1)});
              } else {
                if (r[s].charAt(0) === '4' && e.setfont[4]) {
                  u.push({font: e.setfont[4], text: r[s].substring(1)});
                } else {
  ...
  window.ABCJS.parse.parseDirective.addDirective = function(R) {
    ...
    switch (ac) {
      ...
      case 'score':
        ...
        var ag = function(am, aj, al, ak, ai) {
          if (aj || e.staves.length === 0) {
            e.staves.push({index: e.staves.length, numVoices: 0});
          }
          ...
          if (e.voices[am] === undefined) {
            e.voices[am] = {staffNum: t.index, index: t.numVoices};
            t.numVoices++;
          }
        };
        var T = false;
        ...
        var af = function() {
          ...
          if (D) {
            ...
            if (D.staffNum > 0) {
              if (
                e.staves[D.staffNum - 1].connectBarLines === 'start' ||
                e.staves[D.staffNum - 1].connectBarLines === 'continue'
              ) {
                t = 'continue';
              }
            }
      ...
      case 'footer':
        ...
        var C = {};
        if (L.length === 1) {
          C = {left: '', center: L[0], right: ''};
        } else {
          if (L.length === 2) {
            C = {left: L[0], center: L[1], right: ''};
          } else {
            C = {left: L[0], center: L[1], right: L[2]};
          }
        }
        if (L.length > 3) {
...
window.ABCJS.parse.ParseHeader = function(c, e, a, d) {
  ...
  this.setTempo = function(o, f, i) {
    try {
      ...
      return {type: n ? 'delaySet' : 'immediate', tempo: j};
    } catch (g) {
      e(g, o, f);
      return {type: 'none'};
    }
    ...
    if (h.length >= k + 5 && h.charAt(k) === '[' && h.charAt(k + 2) === ':') {
      ...
      switch (h.substring(k, k + 3)) {
        ...
        case '[K:':
          ...
          return [n - k + 1 + g];
        case '[P:':
          if (d.lines.length <= d.lineNum) {
            a.partForNextLine = h.substring(k + 3, n);
          } else {
            d.appendElement('part', -1, -1, {title: h.substring(k + 3, n)});
          }
          return [n - k + 1 + g];
        case '[L:':
  ...
  this.letter_to_body_header = function(g, j) {
    if (g.length >= j + 3) {
      switch (g.substring(j, j + 2)) {
        ...
        case 'P:':
          if (d.hasBeginMusic()) {
            d.appendElement('part', -1, -1, {title: g.substring(j + 2)});
          }
          return [g.length];
        case 'L:':
      ...
      if (g.charAt(1) === ':') {
        ...
        if (m !== undefined) {
          if (m === 'unalignedWords') {
            d.addMetaTextArray(
              m,
              window.ABCJS.parse.parseDirective.parseFontChangeLine(c.translateString(c.stripComment(g.substring(2)))),
            );
          } else {
          ...
          switch (g.charAt(0)) {
            ...
            default:
              ...
              return {regular: true, str: g + k};
          }
        }
        if (k.length > 0) {
          return {recurse: true, str: k};
        }
        return {};
      }
    }
    return {regular: true, str: g};
  };
};
if (!window.ABCJS) {
  window.ABCJS = {};
...
(function() {
  ...
  window.ABCJS.parse.parseKeyVoice.standardKey = function(x) {
    ...
    var s = {acc: 'sharp', note: 'c'};
    var w = {acc: 'sharp', note: 'g'};
    var l = {acc: 'sharp', note: 'd'};
    var q = {acc: 'sharp', note: 'A'};
    var v = {acc: 'sharp', note: 'e'};
    var z = {acc: 'sharp', note: 'B'};
    var u = {acc: 'flat', note: 'B'};
    var t = {acc: 'flat', note: 'e'};
    var r = {acc: 'flat', note: 'A'};
    var p = {acc: 'flat', note: 'd'};
    var o = {acc: 'flat', note: 'G'};
    var m = {acc: 'flat', note: 'c'};
    var k = {acc: 'flat', note: 'F'};
    var y = {
      'C#': [n, s, w, l, q, v, z],
      'A#m': [n, s, w, l, q, v, z],
      'G#Mix': [n, s, w, l, q, v, z],
  ...
  window.ABCJS.parse.parseKeyVoice.deepCopyKey = function(l) {
    var k = {accidentals: [], root: l.root, acc: l.acc, mode: l.mode};
  ...
  var a = function(m) {
    ...
    return {mid: k - 6, str: m.substring(l)};
  };
  var h = function(k) {
  ...
  window.ABCJS.parse.parseKeyVoice.parseKey = function(x) {
          ...
          if (s) {
            ...
            for (var z = 0; z < b.key.accidentals.length; z++) {
              for (m = 0; m < s.accidentals.length; m++) {
                if (
                  s.accidentals[m].note &&
                  b.key.accidentals[z].note.toLowerCase() === s.accidentals[m].note.toLowerCase()
                ) {
    ...
    if (t.accs) {
      if (!D.foundKey) {
        D.foundKey = true;
        b.key = {root: 'none', acc: '', mode: '', accidentals: []};
      }
      h(t.accs);
    ...
    while (w.length > 0) {
      switch (w[0].token) {
        ...
        case 'perc':
          ...
          b.clef = {type: y.token, verticalPos: j(y.token, 0)};
          D.foundClef = true;
          break;
        default:
  ...
  window.ABCJS.parse.parseKeyVoice.parseVoice = function(n, x, y) {
    ...
    if (p.startStaff || b.staves.length === 0) {
      b.staves.push({index: b.staves.length, meter: b.origMeter});
      if (!b.score_is_present) {
    ...
    if (p.clef) {
      q.clef = {type: p.clef, verticalPos: p.verticalPos};
    }
    if (p.spacing) {
...
window.ABCJS.parse.tokenizer = function() {
  ...
  this.getKeyPitch = function(j) {
    ...
    switch (j.charAt(h)) {
      case 'A':
        return {len: h + 1, token: 'A'};
      case 'B':
        return {len: h + 1, token: 'B'};
      case 'C':
        return {len: h + 1, token: 'C'};
      case 'D':
        return {len: h + 1, token: 'D'};
      case 'E':
        return {len: h + 1, token: 'E'};
      case 'F':
        return {len: h + 1, token: 'F'};
      case 'G':
        return {len: h + 1, token: 'G'};
    }
    return {len: 0};
  };
  ...
  this.getMode = function(l) {
    var k = function(i, m) {
      while (
        m < i.length &&
        ((i.charAt(m) >= 'a' && i.charAt(m) <= 'z') || (i.charAt(m) >= 'A' && i.charAt(m) <= 'Z'))
      ) {
        m++;
      }
      return m;
    ...
    switch (h) {
      case 'mix':
        return {len: k(l, j), token: 'Mix'};
      case 'dor':
        return {len: k(l, j), token: 'Dor'};
      case 'phr':
        return {len: k(l, j), token: 'Phr'};
      case 'lyd':
        return {len: k(l, j), token: 'Lyd'};
      case 'loc':
        return {len: k(l, j), token: 'Loc'};
      case 'aeo':
        return {len: k(l, j), token: 'm'};
      case 'maj':
        return {len: k(l, j), token: ''};
      case 'ion':
        return {len: k(l, j), token: ''};
      case 'min':
        return {len: k(l, j), token: 'm'};
  ...
  this.getClef = function(o, n) {
    ...
    return {len: m + k.length, token: k, explicit: q};
  };
      ...
      case '|':
        ...
        switch (h.charAt(k)) {
          ...
          case ':':
            ...
            return {len: 1 + j, token: 'bar_left_repeat'};
          default:
  ...
  this.getTokenOf = function(k, j) {
    for (var h = 0; h < k.length; h++) {
      if (j.indexOf(k.charAt(h)) < 0) {
        return {len: h, token: k.substring(0, h)};
      }
    }
    return {len: h, token: k};
  };
  ...
  this.getPitchFromTokens = function(j) {
    ...
    if (h.position === undefined) {
      return {warn: 'Pitch expected. Found: ' + j[0].token};
    }
    j.shift();
    while (j.length) {
      switch (j[0].token) {
  ...
  this.getKeyAccidentals2 = function(j) {
    ...
    while (j.length > 0) {
      ...
      if (j[0].token === '^') {
        ...
        j.shift();
        if (j.length === 0) {
          return {accs: h, warn: 'Expected note name after ' + i};
        }
        switch (j[0].token) {
          case '^':
            i = 'dblsharp';
      ...
      switch (j[0].token.charAt(0)) {
        ...
        case 'G':
          ...
          h.push({acc: i, note: j[0].token.charAt(0)});
  ...
  this.getKeyAccidental = function(l) {
    ...
    switch (l.charAt(h)) {
      ...
      case 'F':
      case 'G':
        return {len: h + 1, token: {acc: k[j], note: l.charAt(h)}};
      case '^':
      case '_':
      case '/':
        j += l.charAt(h);
  ...
  this.getVoiceToken = function(j, n, h) {
    ...
    if (j.charAt(k) === '"') {
      ...
      return {len: l - n + 1, token: j.substring(k, l)};
    }
  };
  var f = {
    '`a': 'à',
  ...
  this.getInt = function(l) {
    ...
    var k = '' + h;
    var j = l.indexOf(k);
    return {value: h, digits: j + k.length};
  };
  ...
  this.getMeasurement = function(k) {
    ...
    if (h.token === '.') {
      j++;
      if (k.length === 0) {
        return {used: j, value: parseInt(i)};
      }
      if (k[0].type === 'number') {
        h = k.shift();
        i = i + '.' + h.token;
        j++;
        if (k.length === 0) {
          return {used: j, value: parseFloat(i)};
        }
      }
      h = k.shift();
    }
    switch (h.token) {
      case 'pt':
        return {used: j + 1, value: parseFloat(i)};
      case 'cm':
        return {used: j + 1, value: (parseFloat(i) / 2.54) * 72};
      case 'in':
        return {used: j + 1, value: parseFloat(i) * 72};
      default:
        k.unshift(h);
        return {used: j, value: parseFloat(i)};
    }
  ...
  for (var e = 0; e < this.children.length; e++) {
    if (ABCJS.write.debugPlacement) {
      if (this.children[e].klass === 'ornament') {
        j.printShadedBox(
          this.x,
          j.calcY(this.children[e].top),
          this.w,
          j.calcY(this.children[e].bottom) - j.calcY(this.children[e].top),
          'rgba(0,0,200,0.3)',
        );
      }
    }
...
(function() {
  ...
  ABCJS.write.AbstractEngraver.prototype.createABCElement = function() {
    ...
    switch (d.el_type) {
      ...
      case 'part':
        ...
        a.addChild(
          new ABCJS.write.RelativeElement(d.title, 0, 0, undefined, {
            type: 'part',
      ...
      default:
        ...
        e.addChild(new ABCJS.write.RelativeElement('element type ' + d.el_type, 0, 0, undefined, {type: 'debug'}));
        f[0] = e;
    }
    return f;
  };
  ...
  ABCJS.write.AbstractEngraver.prototype.createNote = function(X, e, af) {
    ...
    if (X.rest) {
      ...
      if (!af) {
        v = this.createNoteHead(k, ai, {verticalPos: q}, null, 0, -this.roomtaken, null, ak, 0, 1);
      }
      if (v) {
        k.addHead(v);
      }
      ...
      for (
        S = aj === 'down' ? X.pitches.length - 2 : 1;
        aj === 'down' ? S >= 0 : S < X.pitches.length;
        S = aj === 'down' ? S - 1 : S + 1
      ) {
      ...
      if (u) {
        ...
        k.addExtra(
          new ABCJS.write.RelativeElement(null, g, 0, ae, {
            type: 'stem',
            pitch2: ad,
            linewidth: d,
          }),
    ...
    if (X.lyric !== undefined) {
      var W = '';
      window.ABCJS.parse.each(X.lyric, function(c) {
      ...
      k.addCentered(
        new ABCJS.write.RelativeElement(W, 0, l.width, undefined, {
          type: 'lyric',
          position: Q,
    ...
    if (!af && X.gracenotes !== undefined) {
      ...
      for (ab = 0; ab < X.gracenotes.length; ab++) {
        ...
        if (X.gracenotes[ab].acciaccatura) {
          ...
          var Z = n ? 5 : 6;
          k.addRight(
            new ABCJS.write.RelativeElement('flags.ugrace', -ah[ab] + Z, 0, H, {
              scalex: D,
              scaley: D,
            }),
          );
        ...
        if (n) {
          ...
          var s = {
            heads: [b],
            abcelem: {averagepitch: B, minpitch: B, maxpitch: B, duration: C},
          };
          n.add(s);
        } else {
          ae = B + (1 / 3) * D;
    ...
    if (X.barNumber) {
      k.addChild(
        new ABCJS.write.RelativeElement(X.barNumber, -10, 0, 0, {
          type: 'barNumber',
        }),
      );
    }
    for (ab = X.maxpitch; ab > 11; ab--) {
      if (ab % 2 === 0 && !X.rest) {
        k.addChild(
          new ABCJS.write.RelativeElement(null, -2, ABCJS.write.glyphs.getSymbolWidth(ai) + 4, ab, {
            type: 'ledger',
          }),
        );
      }
    ...
    for (ab = 0; ab < w.length; ab++) {
      ...
      k.addChild(
        new ABCJS.write.RelativeElement(null, K - 2, ABCJS.write.glyphs.getSymbolWidth(ai) + 4, w[ab], {
          type: 'ledger',
        }),
      );
    }
      ...
      for (ab = 0; ab < X.chord.length; ab++) {
        ...
        switch (X.chord[ab].position) {
          case 'left':
            ...
            O = X.averagepitch;
            k.addExtra(
              new ABCJS.write.RelativeElement(X.chord[ab].name, P, j + 4, O, {
                type: 'text',
                height: V,
              }),
            );
          ...
          case 'below':
            ...
            for (var I = 0; I < Y.length; I++) {
              k.addRight(
                new ABCJS.write.RelativeElement(Y[I], P, j + ag, undefined, {
                  type: 'text',
                  position: 'below',
                  height: V,
                }),
          ...
          case 'above':
            k.addRight(
              new ABCJS.write.RelativeElement(X.chord[ab].name, 0, j + ag, undefined, {
                type: 'text',
                height: V,
              }),
            );
            ...
            if (X.chord[ab].rel_position) {
              ...
              k.addChild(
                new ABCJS.write.RelativeElement(
                  X.chord[ab].name,
                  P + X.chord[ab].rel_position.x,
                  0,
                  X.minpitch + aa / ABCJS.write.spacing.STEP,
                  {type: 'text', height: V},
                ),
              );
            } else {
              ...
              j = a.width;
              k.addCentered(
                new ABCJS.write.RelativeElement(X.chord[ab].name, P, j, undefined, {
                  type: 'chord',
                  position: f,
                  height: V,
                }),
  ...
  ABCJS.write.AbstractEngraver.prototype.createNoteHead = function(m, A, B, p, h, e, v, q, y, C) {
    ...
    if (A === undefined) {
      m.addChild(
        new ABCJS.write.RelativeElement('pitch is undefined', 0, 0, 0, {
          type: 'debug',
        }),
      );
    } else {
      if (A === '') {
        ...
        if (B.printer_shift) {
          ...
          o =
            p === 'down' ? -ABCJS.write.glyphs.getSymbolWidth(A) * C + s : ABCJS.write.glyphs.getSymbolWidth(A) * C - s;
        }
        var n = {
          scalex: C,
          scaley: C,
        ...
        if (v) {
          ...
          m.addRight(
            new ABCJS.write.RelativeElement(v, r, ABCJS.write.glyphs.getSymbolWidth(v) * C, g, {
              scalex: C,
              scaley: C,
            }),
          );
        ...
        for (; q > 0; q--) {
          ...
          m.addRight(
            new ABCJS.write.RelativeElement(
              'dots.dot',
              d.w + y - 2 + 5 * q,
              ABCJS.write.glyphs.getSymbolWidth('dots.dot'),
              k + a,
            ),
          );
        }
    ...
    if (B.accidental) {
      ...
      m.addExtra(
        new ABCJS.write.RelativeElement(b, x, ABCJS.write.glyphs.getSymbolWidth(b), k, {scalex: C, scaley: C}),
      );
    }
    if (B.endTie) {
      if (this.ties[0]) {
    ...
    if (B.startTie) {
      var u = new ABCJS.write.TieElem(
        d,
        null,
        (this.stemdir === 'down' || p === 'down') && this.stemdir !== 'up',
        this.stemdir === 'down' || this.stemdir === 'up',
        true,
      );
    ...
    if (B.endSlur) {
      for (w = 0; w < B.endSlur.length; w++) {
        ...
        if (this.slurs[z]) {
          ...
          f = new ABCJS.write.TieElem(
            null,
            d,
            p === 'down',
            (this.stemdir === 'up' || p === 'down') && this.stemdir !== 'down',
            false,
          );
...
(function() {
  ...
  ABCJS.write.BeamElem.prototype.layout = function() {
    ...
    var n = i(
      ...
      this.stemHeight,
      this.stemsUp,
      m.abcelem.averagepitch,
      q.abcelem.averagepitch,
      this.isflat,
      this.min,
      this.max,
      this.isgrace,
  ...
  function j(B, s, l, A, n) {
    ...
    for (var w = 0; w < B.length; w++) {
      ...
      var C = z.heads[s ? 0 : z.heads.length - 1];
      var m = C.x + (s ? C.w : 0);
      var o = b(l.startX, l.startY, l.endX, l.endY, m);
      var p = s ? -1.5 : 1.5;
      if (A) {
        p = (p * 2) / 3;
      }
      var k = z.abcelem.duration;
      if (k === 0) {
        k = 0.25;
      }
      for (var t = ABCJS.write.getDurlog(k); t < -3; t++) {
        if (u[-4 - t]) {
          u[-4 - t].single = false;
        } else {
          u[-4 - t] = {
      ...
      for (var v = u.length - 1; v >= 0; v--) {
        if (w === B.length - 1 || ABCJS.write.getDurlog(B[w + 1].abcelem.duration) > -v - 4) {
          var r = m;
          var q = o + p * (v + 1);
          if (u[v].single) {
            r = w === 0 ? m + 5 : m - 5;
            q = b(l.startX, l.startY, l.endX, l.endY, r) + p * (v + 1);
          }
          y.push({startX: u[v].x, endX: r, startY: u[v].y, endY: q, dy: n});
          u = u.slice(0, v);
        }
      }
    }
...
(function() {
  ABCJS.write.createClef = function(e) {
    ...
    switch (e.type) {
      ...
      default:
        d.addChild(
          new ABCJS.write.RelativeElement('clef=' + e.type, 0, 0, undefined, {
            type: 'debug',
          }),
        );
    }
      ...
      if (c !== 0) {
        ...
        d.addRight(
          new ABCJS.write.RelativeElement(
            '8',
            b + a,
            ABCJS.write.glyphs.getSymbolWidth('8') * g,
            c > 0 ? d.top + 3 : d.bottom - 1,
            {scalex: g, scaley: g},
          ),
        );
        d.top += 2;
...
(function() {
  ABCJS.write.createKeySignature = function(c) {
    ...
    var a = 0;
    window.ABCJS.parse.each(
      c.accidentals,
      function(e) {
        var d = e.acc === 'sharp' ? 'accidentals.sharp' : e.acc === 'natural' ? 'accidentals.nat' : 'accidentals.flat';
        b.addRight(
          new ABCJS.write.RelativeElement(d, a, ABCJS.write.glyphs.getSymbolWidth(d), e.verticalPos, {
  ...
  ABCJS.write.createTimeSignature = function(c) {
    ...
    if (c.type === 'specified') {
      for (var a = 0; a < c.value.length; a++) {
        if (a !== 0) {
          b.addRight(
            new ABCJS.write.RelativeElement('+', a * 20 - 9, ABCJS.write.glyphs.getSymbolWidth('+'), 6, {
        ...
        if (c.value[a].den) {
          b.addRight(
            new ABCJS.write.RelativeElement(
              c.value[a].num,
              a * 20,
              ABCJS.write.glyphs.getSymbolWidth(c.value[a].num.charAt(0)) * c.value[a].num.length,
              8,
              {
          ...
          b.addRight(
            new ABCJS.write.RelativeElement(
              c.value[a].den,
              a * 20,
              ABCJS.write.glyphs.getSymbolWidth(c.value[a].den.charAt(0)) * c.value[a].den.length,
              4,
              {
      ...
      if (c.type === 'common_time') {
        b.addRight(
          new ABCJS.write.RelativeElement('timesig.common', 0, ABCJS.write.glyphs.getSymbolWidth('timesig.common'), 6, {
        ...
        if (c.type === 'cut_time') {
          b.addRight(
            new ABCJS.write.RelativeElement('timesig.cut', 0, ABCJS.write.glyphs.getSymbolWidth('timesig.cut'), 6, {
...
(function() {
  ...
  var d = function(n, f, q, g, p, j, e) {
    ...
    function s(v, t) {
      ...
      var u = 5;
      q.addChild(
        new ABCJS.write.RelativeElement(v, f / 2, 0, w + i, {
          type: 'decoration',
          klass: 'ornament',
          thickness: 3,
        }),
    ...
    function l(v, u) {
      ...
      q.addChild(
        new ABCJS.write.RelativeElement(v, t, ABCJS.write.glyphs.getSymbolWidth(v), w, {
          klass: 'ornament',
    ...
    for (var g = 0; g < f.length; g++) {
      switch (f[g]) {
        case 'diminuendo(':
          ...
          break;
        case 'diminuendo)':
          j = {start: this.startDiminuendoX, stop: h};
          this.startDiminuendoX = undefined;
          break;
        case 'crescendo(':
          ...
          break;
        case 'crescendo)':
          e = {start: this.startCrescendoX, stop: h};
          this.startCrescendoX = undefined;
          break;
      }
    }
...
ABCJS.write.Glyphs = function() {
  ...
  this.printSymbol = function(c, g, d, f, b) {
    ...
    e[0][1] += c;
    e[0][2] += g;
    return f.path().attr({path: e, stroke: 'none', fill: '#000000', class: b});
  };
...
ABCJS.write.Renderer.prototype.engraveTopText = function(c, f) {
  if (f.metaText.header && this.isPrint) {
    ...
    this.outputTextIf(
      this.padding.left + c / 2,
      f.metaText.header.center,
      'headerfont',
      'header meta-top',
      0,
      null,
...
ABCJS.write.Renderer.prototype.engraveExtraText = function(c, e) {
  ...
  if (e.metaText.unalignedWords) {
    ...
    this.outputTextIf(
      this.padding.left + ABCJS.write.spacing.INDENT,
      d,
      'wordsfont',
      'meta-bottom',
      this.spacing.words,
...
ABCJS.write.Renderer.prototype.printStaveLine = function(x1, x2, pitch, klass) {
  ...
  var pathString = ABCJS.write.sprintf(
...
ABCJS.write.Renderer.prototype.renderText = function(h, g, i, f, e, c, d) {
  ...
  if (!d) {
    var j = a.getBBox();
    a.attr({y: g + j.height / 2});
    if (b.font.box) {
      this.paper.rect(j.x - 1, j.y - 1, j.width + 2, j.height + 2).attr({stroke: '#cccccc'});
    }
  }
  if (f === 'debugfont') {
...
ABCJS.write.Renderer.prototype.printHorizontalLine = function(b, e, g) {
  ...
  h = Math.round(h);
  this.paper.text(10, h, '' + Math.round(h)).attr({'text-anchor': 'start', 'font-size': '18px', fill: j, stroke: j});
  var c = 50;
  var a = b;
  var d = ABCJS.write.sprintf('M %f %f L %f %f L %f %f L %f %f z', c, h - k, c + a, h - k, a, h + k, c, h + k);
  this.paper
    .path()
    .attr({path: d, stroke: 'none', fill: j, class: this.addClasses('staff')})
    .toBack();
  for (var f = 1; f < b / 100; f++) {
    d = ABCJS.write.sprintf(
      'M %f %f L %f %f L %f %f L %f %f z',
      f * 100 - k,
      h - 5,
      f * 100 - k,
...
ABCJS.write.Renderer.prototype.printShadedBox = function(b, g, d, a, c, f) {
  var e = this.paper.rect(b, g, d, a).attr({fill: c, stroke: c});
  if (f) {
...
ABCJS.write.Renderer.prototype.printVerticalLine = function(a, e, d) {
  ...
  this.paper
    .path()
    .attr({path: c, stroke: 'none', fill: f, class: this.addClasses('staff')})
    .toBack();
...
ABCJS.write.StaffGroupElement.prototype.draw = function(l) {
  ...
  if (ABCJS.write.debugPlacement) {
    var a = [
      'rgba(207,27,36,0.4)',
      'rgba(168,214,80,0.4)',
      'rgba(110,161,224,0.4)',
      'rgba(191,119,218,0.4)',
      'rgba(195,30,151,0.4)',
      'rgba(31,170,177,0.4)',
      'rgba(220,166,142,0.4)',
    ];
    b = function(j, o) {
      if (j.positionY[o]) {
  ...
  for (var d = 0; d < this.staffs.length; d++) {
    ...
    if (ABCJS.write.debugPlacement) {
      ...
      l.printShadedBox(
        l.padding.left,
        l.calcY(f.originalTop),
        l.controller.width,
        l.calcY(f.originalBottom) - l.calcY(f.originalTop),
        'rgba(0,0,0,0.1)',
      );
...
(function() {
  ...
  ABCJS.write.TempoElement.prototype.draw = function(w) {
    ...
    if (this.tempo.duration) {
      ...
      var s = w.engraver.createNoteHead(q, E, {verticalPos: B}, 'up', 0, 0, C, t, 0, h);
      q.addHead(s);
      var k;
      if (b < 1) {
        var f = B + (1 / 3) * h;
...
(function() {
  ...
  function b(h, d, f, g, c) {
    ...
    h.printPath({path: e, stroke: '#000000', class: h.addClasses('triplet')});
  }
