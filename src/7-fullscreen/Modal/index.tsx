...
import {rule, lightTheme as theme, useTheme, ZINDEX} from 'nano-theme';
...
const bordersContractClass = rule({
  boxShadow:
...
const classNameContent = rule({
  pos: 'relative',
  z: 2,
  pad: `${theme.space(3) + theme.space(1)}px ${theme.space(1)}px  ${theme.space(2)}px`,
});
...
export const Modal: React.FC<ModalProps> = ({
  ...
  const contentClasses =
    ...
    className +
    (noBorders ? '' : contrast ? bordersContractClass : bordersClass) +
  ...
  return (
    <BaseModalUntyped
      ...
      onEsc={onEsc as any}
      color={color ? color : theme.isLight ? (contrast ? 'rgba(0,0,64,.4)' : 'rgba(255,255,255,.8)') : 'rgba(0,0,0,.4)'}
    >
      {content}
    </BaseModalUntyped>
  );
