...
import {lightTheme as theme, Scale, rule, useTheme, toColor} from 'nano-theme';
...
export const Avatar: React.FC<AvatarProps> = (allProps) => {
  ...
  if (showText && name && !emoji && !post && !(grey || del) && !lightGrey && !transparent && !color) {
    props.style.background = toColor(id || name);
  } else if (post) {
    props.style.background = theme.g(0.1, 0.08);
  ...
  if (isPrivate || badge || isOP || !!bottomRight) {
    ...
    if (isPrivate) {
      ...
      const lockStyle: React.CSSProperties = {
        ...
        top: size * -0.1,
        left: size * -0.1,
        background: props.style.background || toColor(id || name || '') || theme.color.sem.negative[0],
        width: lockSize + 'px',
        height: lockSize + 'px',
        borderRadius: '50%',
        fill: '#fff',
      ...
      badgeElement = (
        <span
          style={{
            ...
            top: size * -0.035,
            right: size * -0.035,
            background: theme.color.sem.warning[0],
